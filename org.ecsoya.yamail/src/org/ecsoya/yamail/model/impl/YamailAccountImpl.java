...
public class YamailAccountImpl extends MinimalEObjectImpl.Container implements
	...
	public YamailFolder getFolder(Object nameOrType) {
		...
		if (nameOrType instanceof FolderType) {
			for (YamailFolder yamailFolder : folders) {
				if (nameOrType == yamailFolder.getType()) {
					return yamailFolder;
				}
			}
		} else if (nameOrType instanceof String) {
			for (YamailFolder yamailFolder : folders) {
				if (((String) nameOrType).equalsIgnoreCase(yamailFolder
						.getName())) {
					return yamailFolder;
				}
			}
	...
	public Object eInvoke(int operationID, EList<?> arguments)
		...
		switch (operationID) {
		case YamailPackage.YAMAIL_ACCOUNT___GET_FOLDER__OBJECT:
			return getFolder(arguments.get(0));
		}
